...
def edit_event_fields_keyboard(event_id, source, page):
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="✅ Сохранить", callback_data=f"editfield:save:{event_id}:{source}:{page}")],
...
async def handle_choose_field(callback: CallbackQuery, state: FSMContext):
    ...
    if field == "save":
        ...
        conn = await asyncpg.connect(DATABASE_URL)
        event_service = EventService(conn)
        if fields:  # обновляем только если были изменения
...
@router.message(EventEdit.editing_organizer)
async def save_organizer(message: Message, state: FSMContext):
    ...
    await state.update_data(fields=fields)
    await state.set_state(EventEdit.choosing_field)
    await message.answer("Организатор обновлён. Что ещё изменить?", reply_markup=edit_event_fields_keyboard(data["event_id"], data["source"], data["page"]))
